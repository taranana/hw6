...
// To be completed
template<typename K, typename V, typename Prober, typename Hash, typename KEqual>
void HashTable<K,V,Prober,Hash,KEqual>::insert(const ItemType& p) //code is super messy because i tried so many different things without realizing i just forgot to make the file properly again
{
    ...
    if(loc == npos) {
        throw std::logic_error("no free slot");
    }
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    if(mIndex_ >= sizeof(CAPACITIES)/sizeof(HASH_INDEX_T)) {
        throw std::logic_error("capacity error");
    }
    ...
    for(size_t i = 0; i < old.size(); i++) {
        if(old[i] != nullptr && !old[i]->deleted) {
            HASH_INDEX_T loc = probe(old[i]->item.first);

            if(loc == Prober::npos) {
                throw std::logic_error("failed");
            }

            table_[loc] = new HashItem(old[i]->item);
            numItems_++;
        }
